...
class UDPBridgeConfig:
    ...
    fps: float # the required FPS of transfer
    nb_markers: int # number of markers to track
    side: str  # "plane", "top" or "front"
    sort: str  # "y" or "z", only for front camera

    # UDP settings
    remote_ip: str # IP address of the remote host 
    remote_port: int # UDP port Remote host listens on (Python -> Remote host)
    local_port: int # UDP port Python listens on (Remote host -> Python)
    bind_port: int # Local port used for sending
    recv_timeout: float # Socket timeout in seconds while waiting for a reply
    camera_only: bool = False # Only send camera data, the motors command will be sent as 0 and no command will be applied to the motors
    motors_only: bool = False # Only send motor data, the markers position will be sent as 0 and the camera will not be read
